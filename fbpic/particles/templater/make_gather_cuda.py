...
def make_gather_cuda( Nm, module_file='./_gather_cuda_.py',
                 template_file='template_gather_cuda.py'):
    ...
    Args['B_docstr'] = '\n\n'.join([B_docstr.format(m) for m in mrange])

    Args['add_mode_lin'] = ''.join([mode_lin_src.format(m) for m in mrange])
    Args['add_mode_cub'] = ''.join([mode_cub_src.format(m) for m in mrange])
